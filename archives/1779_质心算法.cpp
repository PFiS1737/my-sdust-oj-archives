...
//
// Hint
// 当函数返回的对象给另一个对象进行赋值时，如果函数返回值是一个局部变量，则不会调用拷贝构造函数。
// 所以，必须在getCentroid中返回一个使用new运算符创建的对象，而不是一个局部对象。
...
struct Point {
  ...
  Point(double x = 0, double y = 0) : x(x), y(y) {
    cout << setprecision(2) << fixed << "The Point (" << x << ", " << y << ") is created!"
         << endl;
  }
...
struct Graph {
  ...
  Graph(Point *pos, int n) : pos(new Point[n]), n(n) {
    cout << "A graph with " << n << " points is created!" << endl;
    copy(pos, pos + n, this->pos);
  }
  ~Graph() {
    delete[] pos;
