...
struct Publication {
  ...
  Publication(double p, int l, string name) : price(p), length(l), name(name) {
    cout << "Call Publication's constructor!" << endl;
  }
  virtual ~Publication() {
    cout << "Call " << name << "'s de-constructor!" << endl;
...
struct Book : Publication {
  Book(double p, int l) : Publication(p, l, "Book") {
...
struct Tape : Publication {
  Tape(double p, int l) : Publication(p, l, "Tape") {
