...
// 输入分多行。
// 第一行M>0表示有M个测试用例。
// 只有的M行，每行开始有一个字母i或d，第二个是正整数N>0。
// 如果第一个字母是i，则表示本行包括N个int类型的数据；如果第一个字母为d，则表示本行有N个double类型的数据。
//
// Output
// 除前6行输出外，其他输出的行数等于M。其中每一行输出3个数据：
// 对应测试用例的最大值、最小值以及和。实数输出定点小数，且只输出2位小数。
...
template <typename T> struct Data {
  ...
  Data<T> operator+(const Data &other) const {
  ...
  friend ostream &operator<<(ostream &os, const Data &data) {
    cout << setprecision(2) << fixed << data.value;
    return os;
  }
};
 
template <typename T> struct GetResult {
  static Data<T> getMax(Data<T> *data, int n) {
    return *max_element(data, data + n);
  }

  static Data<T> getMin(Data<T> *data, int n) {
    return *min_element(data, data + n);
  }

  static Data<T> getSum(Data<T> *data, int n) {
