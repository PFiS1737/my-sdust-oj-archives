...
struct Manager : Person {
  Manager(const string &n) : Person(n, "Manager") {
  }
...
struct Employee : Person {
  Employee(const string &n) : Person(n, "Employee") {
  }

  int pay() {
    return (base * 12 + base * k) * 1000;
  }
};

struct HourlyWorker : Person {
  HourlyWorker(const string &n) : Person(n, "HourlyWorker") {
  }
...
struct CommWorker : Person {
  CommWorker(const string &n) : Person(n, "CommWorker") {
  }
